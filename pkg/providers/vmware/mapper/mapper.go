...
func NewVmwareMapper(vm *object.VirtualMachine, vmProperties *mo.VirtualMachine, hostProperties *mo.HostSystem, credentials *DataVolumeCredentials, mappings *v2vv1alpha1.VmwareMappings, namespace string, osFinder vos.OSFinder) *VmwareMapper {
	return &VmwareMapper{
...
func buildDataVolumeName(targetVMName string, diskId string) string {
	dvName, _ := utils.NormalizeName(targetVMName + "-" + diskId)
	return dvName
}
